...
## Update 2026-09-12 — the tower sits inside the subalgebra landscape

`synthesis/degenerate-subalgebras.md` generalises the erasure theorem to every basis
hyperplane (relative erasure) and shows the {CD,M} tower is one strand of a larger, stable
landscape generated by *relative* mirrors: every M^b CD^a(O) is a basis hyperplane of
CD^{a+b+1}(O), the 31 hyperplanes of A_6 through its top unit are all T, and the
non-tower hyperplanes (X16.2, X32.3/4/5) are mirrors with respect to non-founding copies.
The "Der = g₂ at every node" statement below is a statement about tower nodes only: the
degenerate hyperplanes have Der = so(4).
